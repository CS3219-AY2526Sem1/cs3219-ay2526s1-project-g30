...
interface MultiSelectProps
	...
	/**
	 * If true, allows users to create custom options from the search input.
	 * When enabled, a "Create" button appears if the search input doesn't match any option.
	 * Optional, defaults to false.
	 */
	allowCustomOptions?: boolean;

	/**
	 * Callback function triggered when a custom option is created.
	 * Receives the custom value. You can use this to persist or log the custom entry.
	 * Optional, only used when allowCustomOptions is true.
	 */
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
	(
		...
		const [customOptions, setCustomOptions] = React.useState<MultiSelectOption[]>([]);
		...
		const getAllOptions = React.useCallback((): MultiSelectOption[] => {
			...
			// Include custom options in the full list
		...
		const filteredOptions = React.useMemo(() => {
			...
			const hasSearch = searchable && searchValue;
			
			if (!hasSearch) {
				// No search: return original structure
				if (isGroupedOptions(options)) {
					// When there are groups, keep them but append custom options
					// This will be handled in rendering
					return options as MultiSelectOption[] | MultiSelectGroup[];
				}
				// Flat options: merge with custom
				return [...(options as MultiSelectOption[]), ...customOptions];
			}
			
			// With search: flatten everything and filter
			if (options.length === 0 && customOptions.length === 0) return [];
			
			let baseOptions: MultiSelectOption[];
			if (isGroupedOptions(options)) {
				baseOptions = options.flatMap((group) => group.options);
			} else {
				baseOptions = options as MultiSelectOption[];
			}
			
			const filtered = [...baseOptions, ...customOptions].filter(
				(option) =>
					option.label.toLowerCase().includes(searchLower) ||
					option.value.toLowerCase().includes(searchLower)
			);
			
			return filtered as MultiSelectOption[] | MultiSelectGroup[];
		...
		const toggleOption = (optionValue: string) => {
			...
			// Remove custom option from customOptions if it's being deselected
			if (selectedValues.includes(optionValue) && !newSelectedValues.includes(optionValue)) {
				const isCustomOption = customOptions.some((opt) => opt.value === optionValue);
				if (isCustomOption) {
		...
		const handleClear = () => {
			...
			setSelectedValues([]);
			onValueChange([]);
			// Clear all custom options when clearing selection
			setCustomOptions([]);
		};
												...
												return (
													<Badge
														...
														<div
															...
															className={cn(
																"ml-1 h-4 w-4 cursor-pointer hover:bg-white/20 rounded-sm flex items-center justify-center focus:outline-none focus:ring-1 focus:ring-white/50",
																responsiveSettings.compactMode && "ml-0.5"
															)}>
															<XCircle
																className={cn(
																	"h-3 w-3",
						...
						<Command>
							...
							<CommandList
								...
								{filteredOptions.length === 0 && (
									allowCustomOptions && searchValue ? (
										<CommandGroup>
											<CommandItem
												onSelect={() => {
													if (!selectedValues.includes(searchValue)) {
														// Add custom option to the internal list
														const newCustomOption: MultiSelectOption = {
															label: searchValue,
															value: searchValue,
														};
														setCustomOptions((prev) => [...prev, newCustomOption]);
														setSelectedValues([...selectedValues, searchValue]);
														onValueChange([...selectedValues, searchValue]);
														onCreateOption?.(searchValue);
														setSearchValue("");
														announce(`Added custom option: ${searchValue}`);
													}
												}}
												role="option"
												aria-label={`Create "${searchValue}"`}
